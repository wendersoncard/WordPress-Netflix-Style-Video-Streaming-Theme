...
if (!class_exists('MultiPostThumbnails')) {
	...
	class MultiPostThumbnails {
		...
		public function enqueue_admin_scripts( $hook ) {
			...
			if (version_compare($wp_version, '3.5', '<')) {	
				add_thickbox();
				wp_enqueue_script( "mpt-featured-image", get_bloginfo('template_url') . '/production/js/multi-post-thumbnails-admin.js', array( 'jquery', 'media-upload' ) );
			} else { // 3.5+ media modal
				wp_enqueue_media( array( 'post' => ( $post_ID ? $post_ID : null ) ) );
				wp_enqueue_script( "mpt-featured-image", get_bloginfo('template_url') . '/production/js/multi-post-thumbnails-admin.js', array( 'jquery', 'set-post-thumbnail' ) );
				wp_enqueue_script( "mpt-featured-image-modal", get_bloginfo('template_url') . '/production/js/media-modal.js', array( 'jquery', 'media-models' ) );				
			}
			
			wp_enqueue_style( "mpt-admin-css", get_bloginfo('template_url') . '/production/css/multi-post-thumbnails-admin.css' );
		}
